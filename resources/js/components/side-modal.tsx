...
const SideModal: React.FC<ModalProps> = ({
    ...
    return (
        <div
            className={`modal-backdrop fixed inset-0 z-[50] flex h-full translate-y-full items-end justify-end overflow-y-auto overflow-x-hidden bg-black bg-opacity-50 backdrop-blur-[2px] transition-transform duration-300 xs:items-end md:translate-x-full md:translate-y-0 ${
                hidden ? '!translate-y-0 md:!translate-x-0' : ''
            }`}
            onClick={handleClose}
        >
            <div className={`relative mr-0 flex h-full max-h-full w-full max-w-full flex-col overflow-hidden rounded-t-lg border bg-white shadow-lg xs:rounded-none ${sizeClassName}`}>
                ...
                <div className={`wb-scroll-sm ${scrollClassName} relative h-full overflow-y-auto px-4 pb-8 pt-5 md:px-6`}>{bodySlot}</div>
