        ...
        const filtered =
            keyword === ''
                ...
                : data?.filter((datum: Dynamic) => {
                      return datum[label].toLowerCase().includes(keyword.toLowerCase());
                  });

        if (filtered?.length <= 0 && route !== null && keyword.trim().length >= 2) {
    ...
    return (
        <div className={`relative ${className}`}>
            <div
                ...
                onClick={() => setTimeout(() => searchInput?.current?.focus(), 100)}
            >
                <small className="truncate text-sm">{selected?.[label] ?? placeholder}</small>
                <input type="text" value={selected?.[label] ?? ''} onChange={() => ''} className="h-[1px] w-full opacity-0" required={required} />
            </div>

            {createPortal(
                <div id={id} className={`dropdown-menu z-[100] hidden rounded-md border border-neutral-200 bg-white ${dropClassname}`}>
                    <div className="px-2 pb-2 pt-3">
                        <div className="relative flex items-center">
                            <input
                                ...
                                onKeyUp={(e) => e.key === 'Enter' && handleFiltering(query)}
                                autoFocus={true}
                            />
                            {loading && (
                    ...
                    <div className="scrollbar-sm flex max-h-[200px] flex-col overflow-auto">
                        <span className="w-full">
                            ...
                            <label
                                ...
                                className="block border-0 px-4 py-2 text-left text-sm hover:bg-neutral-100 peer-checked:bg-neutral-200"
                            >
                        ...
                        {filteredData?.map((item: Dynamic, index: number) => (
                            <span key={index} className="w-full">
                                <input
                                    ...
                                    checked={selected?.[optionValue] == item[optionValue]}
                                    className="peer hidden"
                                />
                                <label
                                    htmlFor={`logic-select-${id}-${index}`}
                                    className={`block border-0 px-4 py-2 text-left text-sm hover:bg-neutral-100 peer-checked:bg-neutral-200 ${lineClassname}`}
                                >
                                    {LineRender ? <LineRender item={item} /> : <span>{item[label]}</span>}
                                </label>
                            </span>
                        ))}
                        ...
                        {filteredData?.length <= 0 && !loading && (
                            <div className="flex flex-col items-center py-3">
                                ...
                                <div className="font-medium text-sm capitalize text-center text-zinc-400">{noRecordText}</div>
                                {noRecordActionClick && (
                                    <button onClick={(e) => noRecordActionClick(e)} className="btn-pink !cursor-pointer my-2 !py-1.5">
                                        {noRecordActionText}
                                    </button>
                                )}
                            </div>
