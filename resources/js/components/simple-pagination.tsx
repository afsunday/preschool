...
const SimplePagination: React.FC<PaginationProps> = ({ pageable = {}, ...props }) => {
    return (
        <div className={`flex items-center justify-between ${props.className}`}>
            <div className="text-base font-medium text-zinc-700">Page {pageable?.current_page}</div>

            <div className="mt-2 inline-flex">
                <Link
                    prefetch={['mount', 'hover']}
                    cacheFor="5s"
                    href={pageable?.prev_page_url ? pagingUrl(pageable.prev_page_url) : '#'}
                ...
                <Link
                    prefetch={['mount', 'hover']}
                    cacheFor="5s"
                    href={pageable?.next_page_url ? pagingUrl(pageable.next_page_url) : '#'}
