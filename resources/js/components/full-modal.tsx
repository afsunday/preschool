...
const FullModal: React.FC<ModalProps> = ({ hidden, sizeClassName = 'xs:max-w-[450px]', dismissable = true, onClose, children }) => {
    ...
    const findSlot = (name: string) =>
        childrenArray.find((child) => React.isValidElement(child) && (child.props as Record<string, unknown>).title === name);
    ...
    return (
        <div
            className={`modal-backdrop fixed inset-0 z-[50] flex h-full translate-y-full items-end justify-center overflow-y-auto overflow-x-hidden bg-black bg-opacity-50 transition-transform duration-300 md:translate-y-full ${
                hidden ? '!translate-y-0' : ''
            }`}
            onClick={handleClose}
        >
            <div className={`relative flex max-h-full w-full max-w-full flex-col overflow-hidden rounded-t-lg border bg-white shadow-lg xs:rounded-none md:h-full ${sizeClassName}`}>
