...
<header x-data="{ open: false, scrolled: false }"
        ...
        class="fixed inset-x-0 top-0 z-50 border-b border-wodi-pink/20 transition-colors duration-300"
        :class="scrolled || open
    ...
    <div class="mx-auto flex h-14 max-w-[1400px] items-center gap-8 px-5 lg:px-8">
        ...
        <nav class="hidden flex-1 items-center gap-7 lg:flex">
            @foreach ($navItems as $item)
                ...
                <a href="{{ $item['url'] }}"
                   @class([
                       'relative whitespace-nowrap text-[13px] font-medium transition-colors',
                    ...
                    @if ($isActive)
                        <span class="absolute -bottom-2 left-0 h-0.5 w-full rounded-full bg-wodi-pink"></span>
                    @endif
                </a>
            @endforeach
        </nav>
        ...
        <div class="ml-auto hidden shrink-0 items-center gap-3 lg:flex">
            ...
            <a href="#resources"
               class="rounded-full border border-wodi-pink bg-white/40 px-4 py-2 text-[11px] font-medium text-wodi-pink transition-colors hover:bg-wodi-pink hover:text-white">
